...
class _GooglePlacesAutoCompleteTextFormFieldState
  ...
  Future<void> getLocation(String text) async {
    ...
    if (kDebugMode) {
      print("Getting Location from: $url");
    }
      ...
      itemBuilder: (BuildContext context, int index) => InkWell(
        onTap: () async {
          if (index < allPredictions.length) {
            var selectedData = allPredictions[index];
            if (widget.isLatLngRequired) {
              getPlaceDetailsFromPlaceId(selectedData);
            }
            if (widget.itmClick != null) {
              widget.itmClick!(selectedData);
            }
  ...
  Future<void> getPlaceDetailsFromPlaceId(Prediction prediction) async {
    try {
      ...
      if (kDebugMode) {
        print("Getting Place Details from: $url");
      }
